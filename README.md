...
- `claude-moe` / `codex-moe`: Claude Code / Codex CLI through the item-level fusion proxies. One fusion instance per config file in `~/.config/ai-harness/fusion/*.json` — claude-moe (:8400, Anthropic protocol, candidates Claude/GPT/GLM) and codex-moe (:8401, Responses protocol, candidates GPT/GLM only; Claude models stay on the Claude harness). Adding another proxy later = drop a new JSON there (any port, either protocol, any candidate mix with counts) and restart the fusion LaunchAgent — no code changes. Sessions start in passthrough; `/moe` (Claude) or the `moe` custom prompt (Codex), or `ai-harness-fusion on <instance>`, switches every subsequent step to fan-out + synthesis. claude-moe keeps your native claude.ai OAuth login (no ANTHROPIC_AUTH_TOKEN; the proxy accepts local requests via `accept_any_local_auth` and swaps in its own upstream key), so claude.ai connectors keep working inside claude-moe sessions.
