...
- `ai-harness-fusion`: control and diagnose the fusion proxy — `status` (health + live counters) / `on`・`off`・`toggle` / `logs` / `errors` (degraded items only) / **`diag`** (layer-by-layer self-diagnosis printing PASS/FAIL with the exact fix per failure — run this first when anything misbehaves).
