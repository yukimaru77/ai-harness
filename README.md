...
- MoE (item-level fusion): the plain `claude` and `codex` sessions now run through the fusion proxies permanently (claude: `ANTHROPIC_BASE_URL` in `~/.claude/settings.json`; codex: `fusion` model provider in `~/.codex/config.toml`). In passthrough mode the proxy is byte-transparent (your own login, models, connectors — verified identical); `/moe` switches the SAME session to fan-out + synthesis from the next step. The old claude-moe/codex-moe entry commands are removed. One fusion instance per config file in `~/.config/ai-harness/fusion/*.json` — claude-moe (:8400, Anthropic protocol, candidates Claude/GPT/GLM) and codex-moe (:8401, Responses protocol, candidates GPT/GLM only; Claude models stay on the Claude harness). Adding another proxy later = drop a new JSON there (any port, either protocol, any candidate mix with counts) and restart the fusion LaunchAgent — no code changes. Synthesizer: Fable 5 (claude side) / GPT-5.5 (codex side); candidates and counts are config (`~/.config/ai-harness/fusion/*.json`).
- `ai-harness-fusion`: control and diagnose the fusion proxies (instance-aware: `... [on|off|toggle|logs|errors|diag] [instance]`) — `status` (health + live counters) / `on`・`off`・`toggle` / `logs` / `errors` (degraded items only) / **`diag`** (layer-by-layer self-diagnosis printing PASS/FAIL with the exact fix per failure — run this first when anything misbehaves).
- `/moe` slash command works in ANY Claude Code session (installed in `~/.claude/commands`): in a `claude-moe` session the switch applies from the next step; in a direct session it flips the global mode and tells you to start `claude-moe` (a live session's backend URL cannot be changed mid-flight).
- Fusion candidates and their counts are config (`fusion-api.json` `candidates`, string or `{"model": ..., "count": N}`; `count: 2` = two independent samples of the same model).
